#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano refactor-cnpj-delete` em 2026-08-27 00:40
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo refactor-cnpj-delete
set -u
set -o pipefail

RUN_ID='api-externa-backend-v1-refactor-cnpj-delete-mtasr57f'
FEATURE='refactor-cnpj-delete'
BASE_BRANCH='spec/refactor-cnpj-delete'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/refactor-cnpj-delete/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-refactor-cnpj-delete-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-refactor-cnpj-delete"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/refactor-cnpj-delete/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/refactor-cnpj-delete-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-001' 'Você executa UMA tarefa da feature "refactor-cnpj-delete" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/refactor-cnpj-delete/spec.md, .spec/features/refactor-cnpj-delete/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-001 — "Validação CNPJ no CnpjFacade + DELETE Corretora"
  critérios/refs: AC-415 (CNPJ formatado é normalizado para 14 dígitos), AC-416 (CNPJ com dígitos verificadores válidos passa a validação), AC-417 (CNPJ com dígitos verificadores inválidos é rejeitado), AC-418 (CNPJ com comprimento diferente de 14 dígitos é rejeitado), AC-419 (CorretoraService delega validação ao CnpjFacade sem duplicar lógica), AC-420 (Corretora ativa é desativada e some das listagens), AC-421 (DELETE em corretora inexistente ou inativa retorna 404)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/infra/facade/CnpjFacade.java, src/main/java/com/apiexternabackend/services/CorretoraService.java, src/main/java/com/apiexternabackend/domains/Corretora.java, src/main/java/com/apiexternabackend/resources/CorretoraResource.java, src/main/resources/db/migration/V8__add_ativo_corretora.sql, src/test/java/com/apiexternabackend/services/CorretoraServiceTest.java, src/test/java/com/apiexternabackend/resources/CorretoraResourceTest.java
  mensagem de commit: "T-001 refactor-cnpj-delete: Validação CNPJ no CnpjFacade + DELETE Corretora"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/refactor-cnpj-delete-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/refactor-cnpj-delete-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-002' 'Você executa UMA tarefa da feature "refactor-cnpj-delete" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/refactor-cnpj-delete/spec.md, .spec/features/refactor-cnpj-delete/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-002 — "DELETE Investidor"
  critérios/refs: AC-422 (Investidor ativo é desativado e some das listagens), AC-423 (DELETE em investidor inexistente ou inativo retorna 404)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/Investidor.java, src/main/java/com/apiexternabackend/services/InvestidorService.java, src/main/java/com/apiexternabackend/resources/InvestidorResource.java, src/main/resources/db/migration/V9__add_ativo_investidor.sql, src/test/java/com/apiexternabackend/services/InvestidorServiceTest.java, src/test/java/com/apiexternabackend/resources/InvestidorResourceTest.java
  mensagem de commit: "T-002 refactor-cnpj-delete: DELETE Investidor"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/refactor-cnpj-delete-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/refactor-cnpj-delete-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-003' 'Você executa UMA tarefa da feature "refactor-cnpj-delete" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/refactor-cnpj-delete/spec.md, .spec/features/refactor-cnpj-delete/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-003 — "DELETE Ação"
  critérios/refs: AC-424 (Ação ativa é desativada e some do catálogo), AC-425 (DELETE em ação inexistente ou inativa retorna 404)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/Acao.java, src/main/java/com/apiexternabackend/services/AcaoService.java, src/main/java/com/apiexternabackend/resources/AcaoResource.java, src/main/resources/db/migration/V10__add_ativo_acao.sql, src/test/java/com/apiexternabackend/services/AcaoServiceTest.java, src/test/java/com/apiexternabackend/resources/AcaoResourceTest.java
  mensagem de commit: "T-003 refactor-cnpj-delete: DELETE Ação"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/refactor-cnpj-delete-faixa-3' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/refactor-cnpj-delete/executar-tarefas.sh --gate"
      exit 0
    fi
