#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano catalogo-acao` em 2026-08-22 23:19
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo catalogo-acao
set -u
set -o pipefail

RUN_ID='api-externa-backend-v1-catalogo-acao-mt503m86'
FEATURE='catalogo-acao'
BASE_BRANCH='spec/catalogo-acao'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/catalogo-acao/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-catalogo-acao-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-catalogo-acao"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/catalogo-acao/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/catalogo-acao-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-200' 'Você executa UMA tarefa da feature "catalogo-acao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-acao/spec.md, .spec/features/catalogo-acao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-200 — "Entidade Acao, enum de mercado, repositório e migration"
  critérios/refs: AC-204 (Ticker duplicado é impedido (RN-A01, RF12))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/Acao.java, src/main/java/com/apiexternabackend/domains/enums/Mercado.java, src/main/java/com/apiexternabackend/repositories/AcaoRepository.java, src/main/resources/db/migration/V4__create_acao.sql
  mensagem de commit: "T-200 catalogo-acao: Entidade Acao, enum de mercado, repositório e migration"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/catalogo-acao-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-200
  return 0
}
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/catalogo-acao-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-201' 'Você executa UMA tarefa da feature "catalogo-acao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-acao/spec.md, .spec/features/catalogo-acao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-201 — "DTOs e mapper da ação"
  critérios/refs: AC-201 (Ticker brasileiro válido é cadastrado com cotação e horário (RN-A03, RN-Q01)), AC-202 (Ticker americano válido usa a fonte US (RN-A03)), AC-206 (Listar ações (RF09)), AC-207 (Buscar ação por ticker (RF10))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/dtos/AcaoRequestDTO.java, src/main/java/com/apiexternabackend/domains/dtos/AcaoResponseDTO.java, src/main/java/com/apiexternabackend/mappers/AcaoMapper.java
  mensagem de commit: "T-201 catalogo-acao: DTOs e mapper da ação"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/catalogo-acao-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-201
  return 0
}
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/catalogo-acao-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-202' 'Você executa UMA tarefa da feature "catalogo-acao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-acao/spec.md, .spec/features/catalogo-acao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-202 — "Adapter de cotação (isolamento, Item 15)"
  critérios/refs: AC-205 (O sistema direciona a busca conforme o mercado (RN-A03, RN-A06)), AC-209 (Toda cotação exibida acompanha o horário de obtenção (RN-Q01))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/infra/adapter/CotacaoAdapter.java, src/main/java/com/apiexternabackend/infra/adapter/CotacaoResultado.java
  mensagem de commit: "T-202 catalogo-acao: Adapter de cotação (isolamento, Item 15)"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/catalogo-acao-faixa-3' "$WT" "$st" || return 1
  marcar_concluidas T-202
  return 0
}
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/catalogo-acao-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-203' 'Você executa UMA tarefa da feature "catalogo-acao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-acao/spec.md, .spec/features/catalogo-acao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-203 — "Integração brapi (mercado BR)"
  critérios/refs: AC-201 (Ticker brasileiro válido é cadastrado com cotação e horário (RN-A03, RN-Q01)), AC-203 (Ticker inexistente na fonte impede o cadastro (RN-A02, item 14)), AC-210 (Fonte indisponível retorna a última cotação conhecida (RN-Q05)), AC-211 (Limite de requisições da fonte é tratado com mensagem própria (item 14, RN-Q05))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/infra/client/brapi/BrapiClient.java, src/main/java/com/apiexternabackend/infra/client/brapi/dtos/BrapiResponseDTO.java, src/main/java/com/apiexternabackend/infra/client/brapi/dtos/BrapiResultDTO.java, src/main/java/com/apiexternabackend/infra/adapter/BrapiAdapter.java
  mensagem de commit: "T-203 catalogo-acao: Integração brapi (mercado BR)"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/catalogo-acao-faixa-4' "$WT" "$st" || return 1
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/catalogo-acao-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-204' 'Você executa UMA tarefa da feature "catalogo-acao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-acao/spec.md, .spec/features/catalogo-acao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-204 — "Integração Twelve Data (mercado US)"
  critérios/refs: AC-202 (Ticker americano válido usa a fonte US (RN-A03)), AC-203 (Ticker inexistente na fonte impede o cadastro (RN-A02, item 14)), AC-210 (Fonte indisponível retorna a última cotação conhecida (RN-Q05)), AC-211 (Limite de requisições da fonte é tratado com mensagem própria (item 14, RN-Q05))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/infra/client/twelvedata/TwelveDataClient.java, src/main/java/com/apiexternabackend/infra/client/twelvedata/dtos/TwelveDataResponseDTO.java, src/main/java/com/apiexternabackend/infra/adapter/TwelveDataAdapter.java
  mensagem de commit: "T-204 catalogo-acao: Integração Twelve Data (mercado US)"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/catalogo-acao-faixa-5' "$WT" "$st" || return 1
...
executar_faixa_6() {
  ...
  preparar_worktree 'faixa-6' 'spec/catalogo-acao-faixa-6' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-6' --estado executando --tentativa "$(tentativa 'faixa-6')"
  : > "$LOG_DIR/faixa-6.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-6' 'T-205' 'Você executa UMA tarefa da feature "catalogo-acao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-acao/spec.md, .spec/features/catalogo-acao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-205 — "Serviço da ação"
  critérios/refs: AC-201 (Ticker brasileiro válido é cadastrado com cotação e horário (RN-A03, RN-Q01)), AC-202 (Ticker americano válido usa a fonte US (RN-A03)), AC-203 (Ticker inexistente na fonte impede o cadastro (RN-A02, item 14)), AC-205 (O sistema direciona a busca conforme o mercado (RN-A03, RN-A06)), AC-208 (Atualizar cotação busca preço novo e registra novo horário (RF11, RN-Q02)), AC-209 (Toda cotação exibida acompanha o horário de obtenção (RN-Q01)), AC-210 (Fonte indisponível retorna a última cotação conhecida (RN-Q05)), AC-211 (Limite de requisições da fonte é tratado com mensagem própria (item 14, RN-Q05))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/services/AcaoService.java
  mensagem de commit: "T-205 catalogo-acao: Serviço da ação"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-6' 'spec/catalogo-acao-faixa-6' "$WT" "$st" || return 1
...
executar_faixa_7() {
  ...
  preparar_worktree 'faixa-7' 'spec/catalogo-acao-faixa-7' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-7' --estado executando --tentativa "$(tentativa 'faixa-7')"
  : > "$LOG_DIR/faixa-7.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-7' 'T-206' 'Você executa UMA tarefa da feature "catalogo-acao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-acao/spec.md, .spec/features/catalogo-acao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-206 — "Controller da ação"
  critérios/refs: AC-206 (Listar ações (RF09)), AC-207 (Buscar ação por ticker (RF10)), AC-208 (Atualizar cotação busca preço novo e registra novo horário (RF11, RN-Q02))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/resources/AcaoResource.java
  mensagem de commit: "T-206 catalogo-acao: Controller da ação"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-7' 'spec/catalogo-acao-faixa-7' "$WT" "$st" || return 1
...
executar_faixa_8() {
  ...
  preparar_worktree 'faixa-8' 'spec/catalogo-acao-faixa-8' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-8' --estado executando --tentativa "$(tentativa 'faixa-8')"
  : > "$LOG_DIR/faixa-8.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-8' 'T-207' 'Você executa UMA tarefa da feature "catalogo-acao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-acao/spec.md, .spec/features/catalogo-acao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-207 — "Testes da ação"
  critérios/refs: AC-201 (Ticker brasileiro válido é cadastrado com cotação e horário (RN-A03, RN-Q01)), AC-202 (Ticker americano válido usa a fonte US (RN-A03)), AC-203 (Ticker inexistente na fonte impede o cadastro (RN-A02, item 14)), AC-204 (Ticker duplicado é impedido (RN-A01, RF12)), AC-205 (O sistema direciona a busca conforme o mercado (RN-A03, RN-A06)), AC-206 (Listar ações (RF09)), AC-207 (Buscar ação por ticker (RF10)), AC-208 (Atualizar cotação busca preço novo e registra novo horário (RF11, RN-Q02)), AC-209 (Toda cotação exibida acompanha o horário de obtenção (RN-Q01)), AC-210 (Fonte indisponível retorna a última cotação conhecida (RN-Q05)), AC-211 (Limite de requisições da fonte é tratado com mensagem própria (item 14, RN-Q05))
  arquivos permitidos (e seus testes): src/test/java/com/apiexternabackend/services/AcaoServiceTest.java, src/test/java/com/apiexternabackend/resources/AcaoResourceTest.java
  mensagem de commit: "T-207 catalogo-acao: Testes da ação"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-8' 'spec/catalogo-acao-faixa-8' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/catalogo-acao/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-200"
  echo "  faixa-2  onda 1  T-201"
  echo "  faixa-3  onda 1  T-202"
  echo "  faixa-4  onda 2  T-203"
  echo "  faixa-5  onda 2  T-204"
  echo "  faixa-6  onda 2  T-205"
  echo "  faixa-7  onda 3  T-206"
  echo "  faixa-8  onda 3  T-207"
  echo
