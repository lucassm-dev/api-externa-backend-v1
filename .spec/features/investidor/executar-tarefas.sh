#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano investidor` em 2026-08-22 23:25
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo investidor
set -u
set -o pipefail

RUN_ID='api-externa-backend-v1-investidor-mt50a5ak'
FEATURE='investidor'
BASE_BRANCH='spec/investidor'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/investidor/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-investidor-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-investidor"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/investidor/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/investidor-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-050' 'Você executa UMA tarefa da feature "investidor" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/investidor/spec.md, .spec/features/investidor/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-050 — "Entidade Investidor, repositório e migration"
  critérios/refs: US-051, US-052
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/Investidor.java, src/main/java/com/apiexternabackend/repositories/InvestidorRepository.java, src/main/resources/db/migration/V1__create_investidor.sql
  mensagem de commit: "T-050 investidor: Entidade Investidor, repositório e migration"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/investidor-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/investidor-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-051' 'Você executa UMA tarefa da feature "investidor" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/investidor/spec.md, .spec/features/investidor/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-051 — "DTOs e mapper do investidor"
  critérios/refs: AC-051 (Cadastro com nome, e-mail e CPF cria o investidor), AC-054 (Listar investidores), AC-055 (Buscar investidor por id)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/dtos/InvestidorRequestDTO.java, src/main/java/com/apiexternabackend/domains/dtos/InvestidorResponseDTO.java, src/main/java/com/apiexternabackend/mappers/InvestidorMapper.java
  mensagem de commit: "T-051 investidor: DTOs e mapper do investidor"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/investidor-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/investidor-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-052' 'Você executa UMA tarefa da feature "investidor" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/investidor/spec.md, .spec/features/investidor/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-052 — "Tratamento de erro centralizado (RNF06)"
  critérios/refs: AC-052 (E-mail e CPF são únicos), AC-053 (E-mail ou CPF mal formatado é rejeitado)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/resources/exceptions/GlobalExceptionHandler.java, src/main/java/com/apiexternabackend/resources/exceptions/StandardError.java, src/main/java/com/apiexternabackend/resources/exceptions/ResourceNotFoundException.java, src/main/java/com/apiexternabackend/resources/exceptions/DuplicateResourceException.java, src/main/java/com/apiexternabackend/resources/exceptions/BusinessException.java, src/main/java/com/apiexternabackend/resources/exceptions/ExternalServiceException.java
  mensagem de commit: "T-052 investidor: Tratamento de erro centralizado (RNF06)"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/investidor-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/investidor-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-053' 'Você executa UMA tarefa da feature "investidor" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/investidor/spec.md, .spec/features/investidor/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-053 — "Serviço do investidor"
  critérios/refs: AC-051 (Cadastro com nome, e-mail e CPF cria o investidor), AC-052 (E-mail e CPF são únicos), AC-053 (E-mail ou CPF mal formatado é rejeitado)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/services/InvestidorService.java
  mensagem de commit: "T-053 investidor: Serviço do investidor"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/investidor-faixa-4' "$WT" "$st" || return 1
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/investidor-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-054' 'Você executa UMA tarefa da feature "investidor" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/investidor/spec.md, .spec/features/investidor/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-054 — "Controller do investidor"
  critérios/refs: AC-051 (Cadastro com nome, e-mail e CPF cria o investidor), AC-054 (Listar investidores), AC-055 (Buscar investidor por id)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/resources/InvestidorResource.java
  mensagem de commit: "T-054 investidor: Controller do investidor"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/investidor-faixa-5' "$WT" "$st" || return 1
...
executar_faixa_6() {
  ...
  preparar_worktree 'faixa-6' 'spec/investidor-faixa-6' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-6' --estado executando --tentativa "$(tentativa 'faixa-6')"
  : > "$LOG_DIR/faixa-6.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-6' 'T-055' 'Você executa UMA tarefa da feature "investidor" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/investidor/spec.md, .spec/features/investidor/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-055 — "Testes do investidor"
  critérios/refs: AC-051 (Cadastro com nome, e-mail e CPF cria o investidor), AC-052 (E-mail e CPF são únicos), AC-053 (E-mail ou CPF mal formatado é rejeitado), AC-054 (Listar investidores), AC-055 (Buscar investidor por id)
  arquivos permitidos (e seus testes): src/test/java/com/apiexternabackend/services/InvestidorServiceTest.java, src/test/java/com/apiexternabackend/resources/InvestidorResourceTest.java
  mensagem de commit: "T-055 investidor: Testes do investidor"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-6' 'spec/investidor-faixa-6' "$WT" "$st" || return 1
...
executar_faixa_7() {
  ...
  preparar_worktree 'faixa-7' 'spec/investidor-faixa-7' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-7' --estado executando --tentativa "$(tentativa 'faixa-7')"
  : > "$LOG_DIR/faixa-7.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-7' 'T-056' 'Você executa UMA tarefa da feature "investidor" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/investidor/spec.md, .spec/features/investidor/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-056 — "Configuração base do Feign"
  critérios/refs: US-101, US-201
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/config/FeignConfig.java
  mensagem de commit: "T-056 investidor: Configuração base do Feign"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-7' 'spec/investidor-faixa-7' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/investidor/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-050"
  echo "  faixa-2  onda 1  T-051"
  echo "  faixa-3  onda 1  T-052"
  echo "  faixa-4  onda 2  T-053"
  echo "  faixa-5  onda 2  T-054"
  echo "  faixa-6  onda 2  T-055"
  echo "  faixa-7  onda 3  T-056"
  echo
