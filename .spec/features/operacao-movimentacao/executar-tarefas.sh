#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano operacao-movimentacao` em 2026-08-22 23:19
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo operacao-movimentacao
set -u
set -o pipefail

RUN_ID='api-externa-backend-v1-operacao-movimentacao-mt503ma2'
FEATURE='operacao-movimentacao'
BASE_BRANCH='spec/operacao-movimentacao'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/operacao-movimentacao/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-operacao-movimentacao-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-operacao-movimentacao"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/operacao-movimentacao/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/operacao-movimentacao-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-400' 'Você executa UMA tarefa da feature "operacao-movimentacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/operacao-movimentacao/spec.md, .spec/features/operacao-movimentacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-400 — "Domínio de operação e posição, repositórios e migration"
  critérios/refs: US-401, US-403
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/Operacao.java, src/main/java/com/apiexternabackend/domains/enums/TipoOperacao.java, src/main/java/com/apiexternabackend/domains/CarteiraAcao.java, src/main/java/com/apiexternabackend/infra/converters/TipoOperacaoConverter.java, src/main/java/com/apiexternabackend/repositories/OperacaoRepository.java, src/main/java/com/apiexternabackend/repositories/CarteiraAcaoRepository.java, src/main/resources/db/migration/V6__create_operacao_posicao.sql
  mensagem de commit: "T-400 operacao-movimentacao: Domínio de operação e posição, repositórios e migration"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/operacao-movimentacao-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-400
  return 0
}
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/operacao-movimentacao-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-401' 'Você executa UMA tarefa da feature "operacao-movimentacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/operacao-movimentacao/spec.md, .spec/features/operacao-movimentacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-401 — "DTOs e mappers de operação"
  critérios/refs: AC-407 (Cada operação gera exatamente uma movimentação (RN-P07)), AC-409 (Histórico traz os campos exigidos (6.6))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/dtos/OperacaoRequestDTO.java, src/main/java/com/apiexternabackend/domains/dtos/OperacaoResponseDTO.java, src/main/java/com/apiexternabackend/domains/dtos/CarteiraAcaoResponseDTO.java, src/main/java/com/apiexternabackend/mappers/OperacaoMapper.java, src/main/java/com/apiexternabackend/mappers/CarteiraAcaoMapper.java
  mensagem de commit: "T-401 operacao-movimentacao: DTOs e mappers de operação"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/operacao-movimentacao-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/operacao-movimentacao-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-402' 'Você executa UMA tarefa da feature "operacao-movimentacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/operacao-movimentacao/spec.md, .spec/features/operacao-movimentacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-402 — "Cálculo de posição e preço médio (recálculo do histórico)"
  critérios/refs: AC-402 (Compras sucessivas geram preço médio ponderado (RN-P02)), AC-405 (Venda que zera a posição a remove, preservando o histórico (RN-P04)), AC-412 (Editar um lançamento recalcula a posição), AC-413 (Excluir um lançamento recalcula a posição)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/services/PosicaoService.java
  mensagem de commit: "T-402 operacao-movimentacao: Cálculo de posição e preço médio (recálculo do histórico)"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/operacao-movimentacao-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/operacao-movimentacao-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-403' 'Você executa UMA tarefa da feature "operacao-movimentacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/operacao-movimentacao/spec.md, .spec/features/operacao-movimentacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-403 — "Serviço de operação (compra e venda)"
  critérios/refs: AC-305 (Só é possível associar ação do mesmo mercado da carteira (RN-P01)), AC-309 (Carteira inativa não recebe novas operações), AC-401 (Compra usa a cotação do momento, nada digitado (RN-Q04, RN-Q02)), AC-402 (Compras sucessivas geram preço médio ponderado (RN-P02)), AC-403 (Só é possível operar ação do mesmo mercado da carteira (RN-P01)), AC-404 (Não é possível vender mais que a posição atual (RN-P03)), AC-405 (Venda que zera a posição a remove, preservando o histórico (RN-P04)), AC-406 (Venda alimenta o resultado realizado por carteira e por ticker), AC-411 (Não existe erro de saldo insuficiente)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/services/OperacaoService.java
  mensagem de commit: "T-403 operacao-movimentacao: Serviço de operação (compra e venda)"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/operacao-movimentacao-faixa-4' "$WT" "$st" || return 1
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/operacao-movimentacao-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-404' 'Você executa UMA tarefa da feature "operacao-movimentacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/operacao-movimentacao/spec.md, .spec/features/operacao-movimentacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-404 — "Consultas: histórico e rentabilidade não realizada"
  critérios/refs: AC-409 (Histórico traz os campos exigidos (6.6)), AC-410 (Rentabilidade não realizada usa a fórmula do PRD (RN-P05))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/services/ConsultaOperacaoService.java
  mensagem de commit: "T-404 operacao-movimentacao: Consultas: histórico e rentabilidade não realizada"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/operacao-movimentacao-faixa-5' "$WT" "$st" || return 1
...
executar_faixa_6() {
  ...
  preparar_worktree 'faixa-6' 'spec/operacao-movimentacao-faixa-6' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-6' --estado executando --tentativa "$(tentativa 'faixa-6')"
  : > "$LOG_DIR/faixa-6.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-6' 'T-405' 'Você executa UMA tarefa da feature "operacao-movimentacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/operacao-movimentacao/spec.md, .spec/features/operacao-movimentacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-405 — "Controller de operação"
  critérios/refs: AC-407 (Cada operação gera exatamente uma movimentação (RN-P07)), AC-409 (Histórico traz os campos exigidos (6.6)), AC-412 (Editar um lançamento recalcula a posição), AC-413 (Excluir um lançamento recalcula a posição)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/resources/OperacaoResource.java
  mensagem de commit: "T-405 operacao-movimentacao: Controller de operação"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-6' 'spec/operacao-movimentacao-faixa-6' "$WT" "$st" || return 1
...
executar_faixa_7() {
  ...
  preparar_worktree 'faixa-7' 'spec/operacao-movimentacao-faixa-7' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-7' --estado executando --tentativa "$(tentativa 'faixa-7')"
  : > "$LOG_DIR/faixa-7.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-7' 'T-406' 'Você executa UMA tarefa da feature "operacao-movimentacao" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/operacao-movimentacao/spec.md, .spec/features/operacao-movimentacao/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-406 — "Testes de operação"
  critérios/refs: AC-305 (Só é possível associar ação do mesmo mercado da carteira (RN-P01)), AC-309 (Carteira inativa não recebe novas operações), AC-401 (Compra usa a cotação do momento, nada digitado (RN-Q04, RN-Q02)), AC-402 (Compras sucessivas geram preço médio ponderado (RN-P02)), AC-403 (Só é possível operar ação do mesmo mercado da carteira (RN-P01)), AC-404 (Não é possível vender mais que a posição atual (RN-P03)), AC-405 (Venda que zera a posição a remove, preservando o histórico (RN-P04)), AC-406 (Venda alimenta o resultado realizado por carteira e por ticker), AC-407 (Cada operação gera exatamente uma movimentação (RN-P07)), AC-409 (Histórico traz os campos exigidos (6.6)), AC-410 (Rentabilidade não realizada usa a fórmula do PRD (RN-P05)), AC-411 (Não existe erro de saldo insuficiente), AC-412 (Editar um lançamento recalcula a posição), AC-413 (Excluir um lançamento recalcula a posição)
  arquivos permitidos (e seus testes): src/test/java/com/apiexternabackend/services/OperacaoServiceTest.java, src/test/java/com/apiexternabackend/services/PosicaoServiceTest.java, src/test/java/com/apiexternabackend/resources/OperacaoResourceTest.java
  mensagem de commit: "T-406 operacao-movimentacao: Testes de operação"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-7' 'spec/operacao-movimentacao-faixa-7' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/operacao-movimentacao/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-400"
  echo "  faixa-2  onda 1  T-401"
  echo "  faixa-3  onda 1  T-402"
  echo "  faixa-4  onda 2  T-403"
  echo "  faixa-5  onda 2  T-404"
  echo "  faixa-6  onda 2  T-405"
  echo "  faixa-7  onda 3  T-406"
  echo
