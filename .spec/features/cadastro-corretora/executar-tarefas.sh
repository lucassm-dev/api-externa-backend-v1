#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano cadastro-corretora` em 2026-08-22 23:25
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo cadastro-corretora
set -u
set -o pipefail

RUN_ID='api-externa-backend-v1-cadastro-corretora-mt50a5bz'
FEATURE='cadastro-corretora'
BASE_BRANCH='spec/cadastro-corretora'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/cadastro-corretora/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-cadastro-corretora-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-cadastro-corretora"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/cadastro-corretora/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/cadastro-corretora-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-100' 'Você executa UMA tarefa da feature "cadastro-corretora" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/cadastro-corretora/spec.md, .spec/features/cadastro-corretora/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-100 — "Entidade Corretora, repositório e migration"
  critérios/refs: AC-104 (CNPJ duplicado é impedido (RN-C05, RF12))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/Corretora.java, src/main/java/com/apiexternabackend/repositories/CorretoraRepository.java, src/main/resources/db/migration/V2__create_corretora.sql
  mensagem de commit: "T-100 cadastro-corretora: Entidade Corretora, repositório e migration"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/cadastro-corretora-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/cadastro-corretora-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-101' 'Você executa UMA tarefa da feature "cadastro-corretora" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/cadastro-corretora/spec.md, .spec/features/cadastro-corretora/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-101 — "DTOs e mapper da corretora"
  critérios/refs: AC-102 (CNPJ válido busca dados cadastrais e endereço e salva (RF02, RF04, RN-C06)), AC-108 (Listar corretoras (RF05)), AC-109 (Buscar corretora por id (RF06)), AC-110 (Buscar corretora por CNPJ (RF06))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/dtos/CorretoraRequestDTO.java, src/main/java/com/apiexternabackend/domains/dtos/CorretoraResponseDTO.java, src/main/java/com/apiexternabackend/mappers/CorretoraMapper.java
  mensagem de commit: "T-101 cadastro-corretora: DTOs e mapper da corretora"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/cadastro-corretora-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/cadastro-corretora-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-103' 'Você executa UMA tarefa da feature "cadastro-corretora" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/cadastro-corretora/spec.md, .spec/features/cadastro-corretora/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-103 — "Integração BrasilAPI (CNPJ)"
  critérios/refs: AC-102 (CNPJ válido busca dados cadastrais e endereço e salva (RF02, RF04, RN-C06)), AC-103 (CNPJ não encontrado na Receita impede o cadastro (RN-C02)), AC-111 (Fonte externa fora do ar é tratada, não quebra a aplicação)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/infra/client/cnpj/CnpjClient.java, src/main/java/com/apiexternabackend/infra/client/cnpj/dto/CnpjResponseDTO.java, src/main/java/com/apiexternabackend/infra/facade/CnpjFacade.java
  mensagem de commit: "T-103 cadastro-corretora: Integração BrasilAPI (CNPJ)"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/cadastro-corretora-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/cadastro-corretora-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-104' 'Você executa UMA tarefa da feature "cadastro-corretora" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/cadastro-corretora/spec.md, .spec/features/cadastro-corretora/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-104 — "Integração ViaCEP (CEP)"
  critérios/refs: AC-102 (CNPJ válido busca dados cadastrais e endereço e salva (RF02, RF04, RN-C06)), AC-111 (Fonte externa fora do ar é tratada, não quebra a aplicação)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/infra/client/cep/CepClient.java, src/main/java/com/apiexternabackend/infra/client/cep/dto/CepResponseDTO.java, src/main/java/com/apiexternabackend/infra/facade/CepFacade.java
  mensagem de commit: "T-104 cadastro-corretora: Integração ViaCEP (CEP)"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/cadastro-corretora-faixa-4' "$WT" "$st" || return 1
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/cadastro-corretora-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-105' 'Você executa UMA tarefa da feature "cadastro-corretora" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/cadastro-corretora/spec.md, .spec/features/cadastro-corretora/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-105 — "Ingestão da base da CVM"
  critérios/refs: AC-105 (Corretora não autorizada na CVM não é cadastrada (RN-C03)), AC-106 (Falha ao verificar a CVM não vira "reprovada" (RN-C04))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/CvmParticipante.java, src/main/java/com/apiexternabackend/repositories/CvmParticipanteRepository.java, src/main/resources/db/migration/V3__create_cvm_participante.sql, src/main/java/com/apiexternabackend/infra/client/cvm/CvmCorretoraClient.java, src/main/java/com/apiexternabackend/infra/client/cvm/dto/CvmCorretoraResponseDTO.java, src/main/java/com/apiexternabackend/services/CvmIngestaoService.java, src/main/java/com/apiexternabackend/config/CvmIngestaoScheduler.java
  mensagem de commit: "T-105 cadastro-corretora: Ingestão da base da CVM"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/cadastro-corretora-faixa-5' "$WT" "$st" || return 1
...
executar_faixa_6() {
  ...
  preparar_worktree 'faixa-6' 'spec/cadastro-corretora-faixa-6' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-6' --estado executando --tentativa "$(tentativa 'faixa-6')"
  : > "$LOG_DIR/faixa-6.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-6' 'T-106' 'Você executa UMA tarefa da feature "cadastro-corretora" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/cadastro-corretora/spec.md, .spec/features/cadastro-corretora/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-106 — "Verificação de autorização na CVM (facade)"
  critérios/refs: AC-105 (Corretora não autorizada na CVM não é cadastrada (RN-C03)), AC-106 (Falha ao verificar a CVM não vira "reprovada" (RN-C04)), AC-107 (A resposta informa a data da base da CVM usada (8.2))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/infra/facade/CvmFacade.java
  mensagem de commit: "T-106 cadastro-corretora: Verificação de autorização na CVM (facade)"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-6' 'spec/cadastro-corretora-faixa-6' "$WT" "$st" || return 1
...
executar_faixa_7() {
  ...
  preparar_worktree 'faixa-7' 'spec/cadastro-corretora-faixa-7' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-7' --estado executando --tentativa "$(tentativa 'faixa-7')"
  : > "$LOG_DIR/faixa-7.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-7' 'T-107' 'Você executa UMA tarefa da feature "cadastro-corretora" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/cadastro-corretora/spec.md, .spec/features/cadastro-corretora/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-107 — "Serviço da corretora"
  critérios/refs: AC-101 (CNPJ mal formatado é rejeitado antes de qualquer chamada externa (RN-C01)), AC-102 (CNPJ válido busca dados cadastrais e endereço e salva (RF02, RF04, RN-C06)), AC-103 (CNPJ não encontrado na Receita impede o cadastro (RN-C02)), AC-104 (CNPJ duplicado é impedido (RN-C05, RF12)), AC-105 (Corretora não autorizada na CVM não é cadastrada (RN-C03)), AC-106 (Falha ao verificar a CVM não vira "reprovada" (RN-C04)), AC-107 (A resposta informa a data da base da CVM usada (8.2)), AC-111 (Fonte externa fora do ar é tratada, não quebra a aplicação)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/services/CorretoraService.java
  mensagem de commit: "T-107 cadastro-corretora: Serviço da corretora"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-7' 'spec/cadastro-corretora-faixa-7' "$WT" "$st" || return 1
...
executar_faixa_8() {
  ...
  preparar_worktree 'faixa-8' 'spec/cadastro-corretora-faixa-8' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-8' --estado executando --tentativa "$(tentativa 'faixa-8')"
  : > "$LOG_DIR/faixa-8.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-8' 'T-108' 'Você executa UMA tarefa da feature "cadastro-corretora" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/cadastro-corretora/spec.md, .spec/features/cadastro-corretora/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-108 — "Controller da corretora"
  critérios/refs: AC-102 (CNPJ válido busca dados cadastrais e endereço e salva (RF02, RF04, RN-C06)), AC-108 (Listar corretoras (RF05)), AC-109 (Buscar corretora por id (RF06)), AC-110 (Buscar corretora por CNPJ (RF06))
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/resources/CorretoraResource.java
  mensagem de commit: "T-108 cadastro-corretora: Controller da corretora"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-8' 'spec/cadastro-corretora-faixa-8' "$WT" "$st" || return 1
...
executar_faixa_9() {
  ...
  preparar_worktree 'faixa-9' 'spec/cadastro-corretora-faixa-9' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-9' --estado executando --tentativa "$(tentativa 'faixa-9')"
  : > "$LOG_DIR/faixa-9.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-9' 'T-109' 'Você executa UMA tarefa da feature "cadastro-corretora" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/cadastro-corretora/spec.md, .spec/features/cadastro-corretora/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-109 — "Testes da corretora"
  critérios/refs: AC-101 (CNPJ mal formatado é rejeitado antes de qualquer chamada externa (RN-C01)), AC-102 (CNPJ válido busca dados cadastrais e endereço e salva (RF02, RF04, RN-C06)), AC-103 (CNPJ não encontrado na Receita impede o cadastro (RN-C02)), AC-104 (CNPJ duplicado é impedido (RN-C05, RF12)), AC-105 (Corretora não autorizada na CVM não é cadastrada (RN-C03)), AC-106 (Falha ao verificar a CVM não vira "reprovada" (RN-C04)), AC-107 (A resposta informa a data da base da CVM usada (8.2)), AC-108 (Listar corretoras (RF05)), AC-109 (Buscar corretora por id (RF06)), AC-110 (Buscar corretora por CNPJ (RF06)), AC-111 (Fonte externa fora do ar é tratada, não quebra a aplicação)
  arquivos permitidos (e seus testes): src/test/java/com/apiexternabackend/services/CorretoraServiceTest.java, src/test/java/com/apiexternabackend/infra/facade/CvmFacadeTest.java, src/test/java/com/apiexternabackend/resources/CorretoraResourceTest.java
  mensagem de commit: "T-109 cadastro-corretora: Testes da corretora"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-9' 'spec/cadastro-corretora-faixa-9' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/cadastro-corretora/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-100"
  echo "  faixa-2  onda 1  T-101"
  echo "  faixa-3  onda 1  T-103"
  echo "  faixa-4  onda 2  T-104"
  echo "  faixa-5  onda 2  T-105"
  echo "  faixa-6  onda 2  T-106"
  echo "  faixa-7  onda 3  T-107"
  echo "  faixa-8  onda 3  T-108"
  echo "  faixa-9  onda 3  T-109"
  echo
