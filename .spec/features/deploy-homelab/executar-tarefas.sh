...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo deploy-homelab
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/deploy-homelab/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-deploy-homelab-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-deploy-homelab"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/deploy-homelab/executar-tarefas.sh --faixa $1"
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/deploy-homelab/executar-tarefas.sh --gate"
      exit 0
    fi
