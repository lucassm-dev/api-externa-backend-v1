#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano carteira` em 2026-08-22 23:19
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo carteira
set -u
set -o pipefail

RUN_ID='api-externa-backend-v1-carteira-mt503m95'
FEATURE='carteira'
BASE_BRANCH='spec/carteira'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/carteira/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-carteira-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/api-externa-backend-v1-carteira"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/carteira/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/carteira-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-300' 'Você executa UMA tarefa da feature "carteira" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/carteira/spec.md, .spec/features/carteira/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-300 — "Entidade Carteira, repositório e migration"
  critérios/refs: AC-303 (Carteira exige uma corretora existente)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/Carteira.java, src/main/java/com/apiexternabackend/repositories/CarteiraRepository.java, src/main/resources/db/migration/V5__create_carteira.sql
  mensagem de commit: "T-300 carteira: Entidade Carteira, repositório e migration"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/carteira-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/carteira-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-301' 'Você executa UMA tarefa da feature "carteira" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/carteira/spec.md, .spec/features/carteira/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-301 — "DTOs e mapper da carteira"
  critérios/refs: AC-301 (Criar carteira definindo um único mercado), AC-306 (Listagem filtra pelo id do investidor informado), AC-307 (Renomear carteira)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/domains/dtos/CarteiraRequestDTO.java, src/main/java/com/apiexternabackend/domains/dtos/CarteiraResponseDTO.java, src/main/java/com/apiexternabackend/mappers/CarteiraMapper.java
  mensagem de commit: "T-301 carteira: DTOs e mapper da carteira"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/carteira-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/carteira-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-302' 'Você executa UMA tarefa da feature "carteira" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/carteira/spec.md, .spec/features/carteira/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-302 — "Serviço da carteira"
  critérios/refs: AC-301 (Criar carteira definindo um único mercado), AC-302 (Um investidor pode ter várias carteiras), AC-303 (Carteira exige uma corretora existente), AC-304 (BR totaliza em reais, US em dólares, nunca somados), AC-306 (Listagem filtra pelo id do investidor informado), AC-307 (Renomear carteira), AC-308 (Excluir carteira é exclusão lógica), AC-310 (Nome de carteira inativa é reutilizável)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/services/CarteiraService.java
  mensagem de commit: "T-302 carteira: Serviço da carteira"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/carteira-faixa-3' "$WT" "$st" || return 1
  marcar_concluidas T-302
  return 0
}
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/carteira-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-303' 'Você executa UMA tarefa da feature "carteira" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/carteira/spec.md, .spec/features/carteira/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-303 — "Controller da carteira"
  critérios/refs: AC-301 (Criar carteira definindo um único mercado), AC-302 (Um investidor pode ter várias carteiras), AC-306 (Listagem filtra pelo id do investidor informado), AC-307 (Renomear carteira), AC-308 (Excluir carteira é exclusão lógica)
  arquivos permitidos (e seus testes): src/main/java/com/apiexternabackend/resources/CarteiraResource.java
  mensagem de commit: "T-303 carteira: Controller da carteira"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/carteira-faixa-4' "$WT" "$st" || return 1
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/carteira-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-304' 'Você executa UMA tarefa da feature "carteira" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/carteira/spec.md, .spec/features/carteira/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-304 — "Testes da carteira"
  critérios/refs: AC-301 (Criar carteira definindo um único mercado), AC-302 (Um investidor pode ter várias carteiras), AC-303 (Carteira exige uma corretora existente), AC-304 (BR totaliza em reais, US em dólares, nunca somados), AC-306 (Listagem filtra pelo id do investidor informado), AC-307 (Renomear carteira), AC-308 (Excluir carteira é exclusão lógica), AC-310 (Nome de carteira inativa é reutilizável)
  arquivos permitidos (e seus testes): src/test/java/com/apiexternabackend/services/CarteiraServiceTest.java, src/test/java/com/apiexternabackend/resources/CarteiraResourceTest.java
  mensagem de commit: "T-304 carteira: Testes da carteira"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/carteira-faixa-5' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/carteira/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-300"
  echo "  faixa-2  onda 1  T-301"
  echo "  faixa-3  onda 1  T-302"
  echo "  faixa-4  onda 2  T-303"
  echo "  faixa-5  onda 2  T-304"
  echo
